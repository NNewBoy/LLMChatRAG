...
class ChatService:
    ...
    async def chat_stream(
        ...
        try:
            ...
            if enable_intent_recognition:
                ...
                if intent_result["intent"] == "rag":
                    ...
                    if results:
                        ...
                        messages = [{"role": "system", "content": f"你是一个智能助手。\n\n当前日期：{now_str()}。当用户询问实时信息时，请使用当前日期进行搜索。"}]
                else:
                    messages = [{"role": "system", "content": f"你是一个智能助手，请友好地回答用户的问题。\n\n当前日期：{now_str()}。当用户询问实时信息时，请使用当前日期进行搜索。"}]
            else:
                messages = [{"role": "system", "content": f"你是一个智能助手，请友好地回答用户的问题。\n\n当前日期：{now_str()}。当用户询问实时信息时，请使用当前日期进行搜索。"}]
            ...
            if settings.agent_framework == "deepagents":
                # ===== DeepAgents 路径 =====
                ...
                async for event_type, data in AgentFactory.create_deep_agent_stream(
                    ...
                    if event_type == "token":
                        full_content += data
                        yield sse_event("token", {"content": data, "message_id": assistant_msg_id})
                    elif event_type == "thinking":
                        thinking_parts.append(data)
                        yield sse_event("thinking", {"content": data})
                    elif event_type == "tool_call":
                        tool_call_parts.append(data)
                        yield sse_event("tool_call", {**data, "timestamp": now_iso()})
                    elif event_type == "tool_result":
                        tool_call_parts.append(data)
                        yield sse_event("tool_result", {**data, "timestamp": now_iso()})
